...
def player_turn(board, player):
    ...
    while pick_row is None or pick_row < 1 or \
        ...
        try:
            pick_row = int(input(colored("Choose a row (1, 2, or 3): ", "yellow")))
            if pick_row < 1 or pick_row > 3:
                print(colored("Invalid row choice. Please enter 1, 2, or 3.", "red"))
    ...
    while units is None or units < 1 or units > len(selected_row) \
        ...
        try:
            units = int(input(colored(f"How many units to take from row {pick_row}? ", "yellow")))
            if units is None or units < 1 or units > len(selected_row) \
                    or units >= units_left(board):
                print(
                    colored(f"Invalid input. Please enter a number between 1 and "
...
def computer_turn(board):
    ...
    take_units(selected_row, units)
    print(colored(f"Computer took {units} units from row {pick_row + 1}.", "yellow"))
...
def play_game(player):
    ...
    while True:
        ...
        if sum(map(len, current_board)) == 1:
            print(colored(f"{player} ","green") + "wins!")
            break
        ...
        if sum(map(len, current_board)) == 1:
            display_board(current_board)
            print(colored("Computer", "red") + " wins!")
            break
...
while True:
    ...
    play_again = input(f"Do you want to play again {players}? (yes/no): ").strip().lower()
    if not play_again.startswith('y'):
        print(colored(f"Goodbye!","cyan")+colored(f" {players}", "blue"))
        break
